...
const SearchableSelect: React.FC<SearchableSelectProps> = ({
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            // Check if click was outside wrapper
            const isOutsideWrapper = wrapperRef.current && !wrapperRef.current.contains(event.target as Node);
            // Check if click was outside dropdown (useful for portal)
            const isOutsideDropdown = dropdownRef.current && !dropdownRef.current.contains(event.target as Node);

            if (isOutsideWrapper && (!usePortal || isOutsideDropdown)) {
                setIsOpen(false);
            }
        }
        ...
        <div className={`relative ${className}`} ref={wrapperRef}>
            ...
            {isOpen && (() => {
                const DropdownContent = (
                    <div
                        ref={dropdownRef}
                        className={`${usePortal ? '' : 'absolute mt-1'} z-50 w-full overflow-hidden rounded-md bg-white text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-meta-4 sm:text-sm`}

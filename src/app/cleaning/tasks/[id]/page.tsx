...
import { ArrowLeft, Edit, MapPin, Clock, User, Building, ExternalLink, RefreshCw, ClipboardList } from 'lucide-react';
...
export default function CleaningDetailPage() {
    ...
    const [data, setData] = useState<CleaningDetail | null>(null);
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response: any = await apiClient.get(`/security/tasks/${id}`);
            setData(response);
        } catch (error: any) {
            console.error("Failed to fetch detail", error);
            Swal.fire({
                title: 'Error',
                text: error.response?.data?.detail || 'Gagal memuat data tugas',
                icon: 'error'
            }).then(() => {
    ...
    return (
        <MainLayout>
            <PageBreadcrumb pageTitle="Detail Tugas Cleaning" />
            ...
            {previewImage && (
                ...
                >
                    ...
                    <button
                        ...
                        onClick={() => setPreviewImage(null)}
                    >
                        <Clock className="rotate-45 w-6 h-6" />
                    </button>
                </div>
            )}
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-xs text-gray-500">Waktu Tugas</p>
                            ...
                            {data.lokasi_absen && (
                                <div className="flex items-start gap-3">
                                    <div className="mt-1 flex h-8 w-8 items-center justify-center rounded-lg bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400">
                                        <MapPin className="h-4 w-4" />
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500">Lokasi Absen</p>
                                        <a
                                            href={`https://maps.google.com/?q=${data.lokasi_absen}`}
                                            target="_blank"
                                            rel="noreferrer"
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-800">
                            <h3 className="font-semibold text-gray-900 dark:text-white">
                                Area Cleaning / Detail Pekerjaan ({data.points.length})
                            </h3>
                            <button
                                onClick={fetchData}
                        ...
                        <div className="divide-y divide-gray-100 dark:divide-gray-800">
                            {data.points.length === 0 ? (
                                <div className="p-8 text-center text-gray-500">
                                    Belum ada detail pekerjaan yang diset.
                                </div>
                            ) : (
                                                ...
                                                <div className="relative aspect-video overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800">
                                                    {point.foto ? (
                                                        ...
                                                        <div className="flex h-full w-full flex-col items-center justify-center text-gray-400 bg-gray-50 dark:bg-gray-800">
                                                            ...
                                                            <span className="text-xs">Belum dikerjakan</span>
                                                        </div>
                                                    )}

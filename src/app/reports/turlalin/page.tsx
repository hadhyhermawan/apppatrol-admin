...
function LaporanTurlalinPage() {
    ...
    return (
        <MainLayout>
            <PageBreadcrumb pageTitle="Laporan Pengaturan Lalu Lintas" />
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6 mb-8">
                <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex gap-3">
                        ...
                        {isPrinting && (
                            <iframe
                                src={`/reports/turlalin/cetak?startDate=${dateStart}&endDate=${dateEnd}&cabang=${filterCabang}&vendorId=${filterVendor}&search=${searchTerm}`}
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-100 text-left dark:bg-gray-800">
                                ...
                                <th className="min-w-[200px] px-4 py-4 font-medium text-black dark:text-white">Waktu In / Out</th>
                                <th className="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">Nomor Polisi</th>
                                <th className="min-w-[200px] px-4 py-4 font-medium text-black dark:text-white">Petugas Bertugas</th>
                                <th className="min-w-[300px] px-4 py-4 font-medium text-black dark:text-white">Keterangan</th>
                            ...
                            {loading ? (
                                ...
                                paginatedData.map((item, idx) => (
                                    <tr key={item.id} className="border-b border-stroke dark:border-strokedark hover:bg-gray-50 dark:hover:bg-meta-4/20">
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] uppercase font-bold text-green-700 bg-green-50 px-1.5 py-0.5 rounded border border-green-200">IN</span>
                                                    <span className="text-black dark:text-white text-sm font-medium">
                                                        {item.jam_masuk ? new Date(item.jam_masuk).toLocaleString('id-ID') : '-'}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-2 pt-2 border-t border-stroke dark:border-strokedark border-dashed">
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="font-bold text-black dark:text-white uppercase text-sm whitespace-nowrap">{item.nomor_polisi}</div>
                                        </td>
                                        <td className="px-4 py-4">
                                            <div className="flex flex-col gap-3">
                                                <div>
                                                    <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-0.5">Petugas Masuk</div>
                                                    <div className="font-semibold text-black dark:text-white text-sm">{item.nama_guard_masuk || '-'}</div>
                                                </div>
                                                {item.nama_guard_keluar && (
                                                    <div>
                                                        <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-0.5">Petugas Keluar</div>
                                                        <div className="font-semibold text-black dark:text-white text-sm">{item.nama_guard_keluar}</div>
                                                    </div>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-sm">
                                            <p className="whitespace-pre-wrap">{item.keterangan || '-'}</p>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex items-center justify-center gap-3">
                                                ...
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    ) : (item.jam_keluar && <span className="text-[10px] text-gray-400 italic">No Photo OUT</span>)}
...
export default withPermission(LaporanTurlalinPage, { permissions: [] });

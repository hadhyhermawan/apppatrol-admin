...
function LaporanTamuPage() {
    ...
    return (
        <MainLayout>
            <PageBreadcrumb pageTitle="Laporan Buku Tamu" />
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6 mb-8">
                <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex gap-3">
                        ...
                        {isPrinting && (
                            <iframe
                                src={`/reports/tamu/cetak?startDate=${dateStart}&endDate=${dateEnd}&cabang=${filterCabang}&vendorId=${filterVendor}&search=${searchTerm}`}
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-100 text-left dark:bg-gray-800">
                                ...
                                <th className="min-w-[200px] px-4 py-4 font-medium text-black dark:text-white">Data Tamu</th>
                                <th className="min-w-[300px] px-4 py-4 font-medium text-black dark:text-white">Keperluan</th>
                                    ...
                                    <tr key={item.id_tamu} className="border-b border-stroke dark:border-strokedark hover:bg-gray-50 dark:hover:bg-meta-4/20">
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex flex-col gap-1">
                                                <div className="font-bold text-black dark:text-white text-sm">{item.nama}</div>
                                                {item.perusahaan && (
                                                    <div className="text-xs text-gray-500">{item.perusahaan}</div>
                                                )}
                                                {item.no_pol && (
                                                    <div className="text-xs font-semibold uppercase mt-1 max-w-fit">No. Polisi: {item.no_pol}</div>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-sm">
                                            <div className="flex flex-col gap-1">
                                                <p className="whitespace-pre-wrap font-medium">{item.keperluan || '-'}</p>
                                                {item.bertemu_dengan && (
                                                    <div className="text-xs text-gray-500 mt-1">Bertemu: {item.bertemu_dengan}</div>
                                                )}
                                                {item.nama_satpam_masuk && (
                                                    <div className="text-[10px] text-gray-400 mt-2 pt-2 border-t border-gray-100 border-dashed">Petugas: {item.nama_satpam_masuk}</div>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-center">
...
export default withPermission(LaporanTamuPage, { permissions: [] });

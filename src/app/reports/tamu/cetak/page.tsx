...
type TamuItem = {
    ...
    nama_satpam_masuk?: string | null;
};

function CetakLaporanTamuContent() {
                ...
                <div id="print-area">
                    ...
                    <div className="flex mb-8 border-b-2 border-gray-800 pb-6 items-center justify-between">
                        ...
                        <div className="text-right">
                            <h2 className="text-xl font-bold text-blue-800 uppercase tracking-widest bg-blue-50 px-4 py-1 rounded-full border border-blue-200 inline-block">Buku Tamu</h2>
                    ...
                    <div className="mb-6">
                        <h3 className="text-lg font-bold text-gray-900 mb-1">Riwayat Buku Tamu</h3>
                    ...
                    <table className="w-full text-xs text-left align-middle border-collapse border border-gray-300">
                        <thead className="bg-gray-100 text-gray-900 border-b-2 border-gray-400">
                            <tr>
                                ...
                                <th className="px-3 py-2 border border-gray-300 w-[20%] font-semibold">Data Tamu</th>
                                <th className="px-3 py-2 border border-gray-300 w-[30%] font-semibold">Keperluan</th>
                                <th className="px-3 py-2 border border-gray-300 text-center w-[15%] font-semibold">Bukti Foto</th>
                            </tr>
                        </thead>
                        <tbody>
                            {data.map((item, key) => (
                                <tr key={item.id_tamu} className={key % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                    ...
                                    <td className="px-3 py-3 border border-gray-300 my-no-break align-top">
                                        <div className="flex flex-col gap-1">
                                            <div className="font-bold text-gray-900 leading-tight uppercase">{item.nama}</div>
                                            {item.perusahaan && (
                                                <div className="text-xs text-gray-600 leading-tight">{item.perusahaan}</div>
                                            )}
                                            {item.no_pol && (
                                                <div className="text-[10px] font-semibold uppercase mt-1 max-w-fit">No. Polisi: {item.no_pol}</div>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-3 py-3 border border-gray-300 my-no-break align-top">
                                        <div className="flex flex-col gap-1">
                                            <p className="whitespace-pre-wrap text-gray-800 font-medium leading-relaxed max-w-[400px]">
                                                {item.keperluan || '-'}
                                            </p>
                                            {item.bertemu_dengan && (
                                                <div className="text-xs text-gray-600 mt-1">Bertemu: {item.bertemu_dengan}</div>
                                            )}
                                            {item.nama_satpam_masuk && (
                                                <div className="text-[10px] text-gray-400 mt-2 pt-2 border-t border-gray-100 border-dashed">Petugas Menangani: {item.nama_satpam_masuk}</div>
                                            )}
                                        </div>
                                    </td>

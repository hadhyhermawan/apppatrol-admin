...
export default function PengajuanIzinPage() {
    ...
    const [selectedDetail, setSelectedDetail] = useState<IzinData | null>(null);
    ...
    const getKodeField = () => {
        switch (activeTab) {
            case 'sakit': return 'kode_izin_sakit';
            case 'cuti': return 'kode_cuti';
            case 'dinas': return 'kode_izin_dinas';
            default: return 'kode_izin';
        }
    };
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const mappedData = Array.isArray(res) ? res.map(item => ({
                ...item,
                kode: item[kodeField]
            })) : [];
            setData(mappedData);
    ...
    const handleCreate = () => {
        setIsEditing(false);
        setFormData({ nik: '', dari: '', sampai: '', keterangan: '' });
        setShowModal(true);
    };

    const handleEdit = async (id: string) => {
        try {
            const res: any = await apiClient.get(`${getApiEndpoint()}/${id}`);
            setFormData({
                nik: res.nik,
                dari: res.dari,
                sampai: res.sampai,
    ...
    const handleViewDetail = async (id: string) => {
        try {
            const res: any = await apiClient.get(`${getApiEndpoint()}/${id}`);
            setSelectedDetail({ ...res, kode: res[getKodeField()] });
    ...
    const handleDelete = (id: string) => {
        Swal.fire({
            title: 'Hapus Data?',
            text: `Data ${getTabTitle()} ini akan dihapus!`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    await apiClient.delete(`${getApiEndpoint()}/${id}`);
    ...
    const handleApprove = async (id: string, approve: boolean) => {
        try {
            await apiClient.post(`${getApiEndpoint()}/${id}/approve`, { approve });
            Swal.fire('Berhasil', approve ? `${getTabTitle()} disetujui` : `${getTabTitle()} ditolak`, 'success');
            fetchData();
        } catch (error) {
    ...
    const handleCancelApprove = async (id: string) => {
        try {
            await apiClient.post(`${getApiEndpoint()}/${id}/cancel-approve`, {});
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && currentId) {
                await apiClient.put(`${getApiEndpoint()}/${currentId}`, formData);
            } else {
                await apiClient.post(getApiEndpoint(), formData);
            }
    ...
    return (
        <MainLayout>
            <PageBreadcrumb pageTitle="Pengajuan Izin" />

            <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                {/* Header */}
                <div className="p-5 sm:p-6 pb-0 mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                {/* TABS */}
                <div className="mb-6 border-b border-stroke dark:border-strokedark px-6">
                    <div className="flex gap-8 overflow-x-auto">
                        <button
                            ...
                            className={`pb-4 text-sm font-medium hover:text-brand-500 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'absen'
                        ...
                        <button
                            ...
                            className={`pb-4 text-sm font-medium hover:text-brand-500 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'sakit'
                                ? 'border-brand-500 text-brand-500'
                                : 'border-transparent text-gray-500 dark:text-gray-400'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`pb-4 text-sm font-medium hover:text-brand-500 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'cuti'
                                ? 'border-brand-500 text-brand-500'
                                : 'border-transparent text-gray-500 dark:text-gray-400'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`pb-4 text-sm font-medium hover:text-brand-500 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'dinas'
                                ? 'border-brand-500 text-brand-500'
                                : 'border-transparent text-gray-500 dark:text-gray-400'
                                }`}
                        >
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    paginatedData.map((item, idx) => (
                                        <tr key={item.kode} className="border-b border-stroke dark:border-strokedark hover:bg-gray-50 dark:hover:bg-meta-4/20">
                                                ...
                                                <div className="flex items-center justify-center gap-2">
                                                    ...
                                                    {item.status === '0' && (
                                                        <>
                                                            ...
                                                            <button onClick={() => handleApprove(item.kode, true)} className="hover:text-green-500 text-gray-500 dark:text-gray-400" title="Approve">
                                                                <CheckCircle className="h-4 w-4" />
                                                            </button>
                                                            <button onClick={() => handleApprove(item.kode, false)} className="hover:text-red-500 text-gray-500 dark:text-gray-400" title="Reject">
                ...
                <div className="fixed inset-0 z-999999 flex items-center justify-center overflow-y-auto bg-black/50 backdrop-blur-sm p-4">
                    <div className="relative w-full max-w-2xl rounded-2xl bg-white p-8 shadow-2xl dark:bg-boxdark">
                        ...
                        <h3 className="mb-6 text-2xl font-bold text-black dark:text-white flex items-center gap-2">
                            <FileText className="w-6 h-6 text-brand-500" />
                            {isEditing ? `Edit ${getTabTitle()}` : `Tambah ${getTabTitle()}`}
                        </h3>
                            ...
                            <div className="flex flex-col gap-6 mb-6">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={4}
                                        className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 text-black outline-none transition focus:border-brand-500 active:border-brand-500 dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-brand-500"
                                        placeholder={`Alasan ${getTabTitle().toLowerCase()}...`}
                                        required
                                    />
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-999999 flex items-center justify-center overflow-y-auto bg-black/50 backdrop-blur-sm p-4">
                    <div className="relative w-full max-w-3xl rounded-2xl bg-white p-8 shadow-2xl dark:bg-boxdark">
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-sm text-gray-500 dark:text-gray-400">Kode</label>
                                    <p className="font-semibold text-black dark:text-white">{selectedDetail.kode}</p>
                                </div>
                                <div>

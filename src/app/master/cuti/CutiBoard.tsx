...
export default function CutiBoard({ data, vendors, onUpdate }: Props) {
    const [isBrowser, setIsBrowser] = useState(false);
    const [columns, setColumns] = useState<Record<string, CutiItem[]>>({});
    ...
    useEffect(() => {
        // Group data
        const initialCols: Record<string, CutiItem[]> = {
            "unassigned": []
        };
        vendors.forEach(v => {
            initialCols[v.value] = [];
    ...
    const onDragEnd = async (result: DropResult) => {
        ...
        try {
            await apiClient.put(`/master/cuti/${updatedItem.kode_cuti}`, {
                kode_cuti: updatedItem.kode_cuti,
                jenis_cuti: updatedItem.jenis_cuti,
                jumlah_hari: updatedItem.jumlah_hari,
            ...
            console.error(error);
            Swal.fire('Gagal!', error.response?.data?.detail || "Gagal mengubah vendor cuti.", 'error');
...
function BoardColumn({ id, title, items, isUnassigned }: { id: string, title: string, items: CutiItem[], isUnassigned?: boolean }) {
    return (
        <div className={`w-[300px] shrink-0 rounded-xl border flex flex-col h-[calc(100vh-250px)] ${isUnassigned ? 'bg-blue-50/50 border-blue-200 dark:bg-blue-900/10 dark:border-blue-800/30' : 'bg-gray-50 border-gray-200 dark:bg-meta-4/50 dark:border-strokedark'}`}>
            <div className={`p-4 border-b flex items-center justify-between sticky top-0 z-10 rounded-t-xl backdrop-blur-md ${isUnassigned ? 'border-blue-200 bg-blue-100/50 dark:border-blue-800/30 dark:bg-blue-900/20' : 'border-gray-200 bg-gray-100/50 dark:border-strokedark dark:bg-[#252b36]/50'}`}>
                <h3 className="font-semibold text-black dark:text-white flex items-center gap-2 text-sm">
                    <Calendar className={`w-4 h-4 ${isUnassigned ? 'text-blue-500' : 'text-brand-500'}`} />
                    {title}
                </h3>
                ...
                {(provided, snapshot) => (
                    ...
                    >
                        {items.map((item, index) => (
                            <Draggable key={item.kode_cuti} draggableId={item.kode_cuti} index={index}>

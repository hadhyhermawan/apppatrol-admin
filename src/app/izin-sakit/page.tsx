...
export default function IzinSakitPage() {
    const [data, setData] = useState<IzinSakit[]>([]);
    ...
    const [selectedDetail, setSelectedDetail] = useState<IzinSakit | null>(null);
    ...
    const filteredData = useMemo(() => {
        return data.filter(item =>
            ...
            item.kode_izin.toLowerCase().includes(searchTerm.toLowerCase())
        );
    }, [data, searchTerm]);
    ...
    const handleDelete = (id: string) => {
        Swal.fire({
            title: 'Hapus Data?',
            text: "Data izin sakit ini akan dihapus!",
            icon: 'warning',
    ...
    return (
        <MainLayout>
            <PageBreadcrumb pageTitle="Izin Sakit" />
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                paginatedData.map((item, idx) => (
                                    <tr key={item.kode_izin} className="border-b border-stroke dark:border-strokedark hover:bg-gray-50 dark:hover:bg-meta-4/20">
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <button onClick={() => handleViewDetail(item.kode_izin)} className="hover:text-blue-500 text-gray-500 dark:text-gray-400" title="Detail">
                                                    <Eye className="h-4 w-4" />
                                                </button>
                                                {item.status === '0' && (
                                                    <>
                                                        <button onClick={() => handleEdit(item.kode_izin)} className="hover:text-yellow-500 text-gray-500 dark:text-gray-400" title="Edit">
                                                            <Edit className="h-4 w-4" />
                                                        </button>
                                                        <button onClick={() => handleApprove(item.kode_izin, true)} className="hover:text-green-500 text-gray-500 dark:text-gray-400" title="Approve">
                                                            <CheckCircle className="h-4 w-4" />
                                                        </button>
                                                        <button onClick={() => handleApprove(item.kode_izin, false)} className="hover:text-red-500 text-gray-500 dark:text-gray-400" title="Reject">
                                                            <XCircle className="h-4 w-4" />
                                                        </button>
                                                    </>
                                                )}
                                                {item.status !== '0' && (
                                                    <button onClick={() => handleCancelApprove(item.kode_izin)} className="hover:text-orange-500 text-gray-500 dark:text-gray-400" title="Cancel Approval">
                                                        <AlertCircle className="h-4 w-4" />
                                                    </button>
                                                )}
                                                <button onClick={() => handleDelete(item.kode_izin)} className="hover:text-red-500 text-gray-500 dark:text-gray-400" title="Hapus">
                ...
                <div className="fixed inset-0 z-999999 flex items-center justify-center overflow-y-auto bg-black/50 backdrop-blur-sm p-4">
                    <div className="relative w-full max-w-2xl rounded-2xl bg-white p-8 shadow-2xl dark:bg-boxdark">
                        ...
                        <h3 className="mb-6 text-2xl font-bold text-black dark:text-white flex items-center gap-2">
                            <FileText className="w-6 h-6 text-brand-500" />
                            {isEditing ? 'Edit Izin Sakit' : 'Tambah Izin Sakit'}
                        </h3>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="font-semibold text-black dark:text-white">{selectedDetail.kode_izin}</p>
                                </div>
                                <div>
